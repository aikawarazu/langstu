...
```bash
python3 backend/scripts/import_nce_notes.py
# 生成 app/data/notes/<册>/<课>.json（276 份）+ app/data/notes-index.js（计数索引）
```

音视频不自托管，前端直连外链；教材笔记按课懒加载（首次打开该课时请求一次）。

## 项目结构

```
app/                      # 站点根目录（GitHub Pages 直接发布）
  index.html              # 入口页（学习台 / 笔记总览 / 目录气泡 / 记笔记浮框）
  css/
    app.css               # 基础样式
    v12.css               # 字幕模式 / 单元行
    textbook.css          # 教材纸页排版
    split.css             # 左右分栏 / 目录气泡 / 锚点条 / 笔记总览 / 浮框
  js/app.js               # 全部主逻辑（无框架、无构建）
  data/
    NCE1~4.js             # 单元 / 音频 / 视频映射数据
    notes.js              # 手写精修课文笔记（优先级最高）
    notes-index.js        # 各课笔记计数索引（脚本生成）
    notes/<册>/<课>.json  # 276 课教材笔记（脚本生成，按课懒加载）
backend/scripts/          # 数据导入 / 派生脚本（开发期使用，非运行期依赖）
  import_nce_notes.py     # 从参考仓库拉取并转换 276 课教材笔记
  import_nce.py 等        # 课文 / 音视频映射派生脚本
docs/                     # 需求 / UX / M1 spec 文档
```
...
- 个人数据（已学标记、我的笔记）存 `localStorage`，不上传
