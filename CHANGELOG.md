...
## [app-v1.5.1] - 2026-09-20

> 版本号说明：`app-v1.5.0` tag 已存在（打在 2026-09-09 的 `87378ad` 上，注解写的是「上一课/下一课按钮移至课程头部右侧」），
> 因此本次不能沿用 1.4.x。同时 `APP` 常量此前一直停在 `1.4.0`（漏改），本次一并补齐到 `1.5.1`。
...
- `app/js/data/registry.js` 的 `APP` 常量 `1.4.0` → `1.5.1`（补齐漏改；站点升级后会自动刷新内置资源网站，老用户即可拿到新数据源地址）。
